...
![Выделенная область в конце файла](image/2.png)
...
Переместил курсор в конец буфера (M->), а также с помощью зажатия(Alt-c).

![Конечное положение курсора в конце буфера файла](image/4.png)

Далее учусь управвлять буферами файла.
...
В каждом из четырёх созданных окон открыл новый буфер (файл) и ввёл несколько строк текста.
...
Переключился в режим поиска (C-s) и найшёл несколько слов, присутствующих в тексте, а именно bin.

Переключился между результатами поиска, нажимая C-s.

![Процесс переключения между рузьтатами поиска](image/8.png)



Вышел из режима поиска, нажав C-g.
Перейдите в режим поиска и замены (M-%), ввел текст bin, который планирую найти и заменить на REPLACE, нажал Enter, а затем ввёл текст для замены. После того как результаты поиска были подсвечены, нажал ! для подтверждения замены.

![Результат замены](image/9.png)



Испробовал другой режим поиска, нажав (M-s o). Он выгодно отличается от обычного режима если требуется найти что-то в очень большом объеме текста, так как предоставляет информацию о искомом тексте сразу с номером строки, следовательно не требует рассматривать результаты, переходя на каждую строку в отдельности в отличие от обычного режима.
...
Интерфейс в котром происходит практически всё взаимодействие пользователя с редактором emacs.
